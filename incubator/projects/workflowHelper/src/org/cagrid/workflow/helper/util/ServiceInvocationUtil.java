...
public class ServiceInvocationUtil {
    ...
    public static Node generateUnsecureRequest(WorkflowInvocationHelperDescriptor workflowDescriptor,
        OperationInputMessageDescriptor input_desc, OperationOutputTransportDescriptor output_desc,
        InputParameter[] paramData) throws Exception {

        return generateRequest(workflowDescriptor, input_desc, output_desc, paramData, null);
    }


    public static Node generateSecureRequest(WorkflowInvocationHelperDescriptor workflowDescriptor,
        OperationInputMessageDescriptor input_desc, OperationOutputTransportDescriptor output_desc,
        InputParameter[] paramData, GlobusCredential proxy) throws Exception {
     ...
     *            Description of operation to invoke
     * @param input_desc
     *            Description of operation's inputs
     * @param output_desc
     *            Description of operation's output
     * @param paramData
     *            Values of operation's parameters
     * @return The operation's output value
     */
    private synchronized static Node generateRequest(final WorkflowInvocationHelperDescriptor workflowDescriptor,
        final OperationInputMessageDescriptor input_desc, final OperationOutputTransportDescriptor output_desc,
        final InputParameter[] paramData, final GlobusCredential proxy) throws Exception {
        ...
        to.addAttribute(new PrefixedQName(new QName("xmlns:wsa")), "http://schemas.xmlsoap.org/ws/2004/03/addressing");
        ...
        from
            .addAttribute(new PrefixedQName(new QName("xmlns:wsa")), "http://schemas.xmlsoap.org/ws/2004/03/addressing");
        SOAPHeaderElement address = new SOAPHeaderElement(new PrefixedQName(new QName("wsa:Address"))); // 'Address'
                                                                                                        // element
        ...
        paramsLoop : for (int i = 0; i < numParams; i++) {
            ...
            /*
             * Convert the user's data into one single tree of SOAPElements. We
             * would need to get rid of the root, that's something like
             * <....Response>. Although, the root is useful to get information
             * about the parameter's type.
             */
            final QName cur_param_qname = input_desc.getInputParam(i).getParamQName();
            SOAPBodyElement curr_param = new org.apache.axis.message.SOAPBodyElement(new QName(cur_param_qname
                .getLocalPart()));
            Iterator root_obj = ConversionUtil.String2SOAPElement(paramData[i].getData());

            /* Add this parameter */
            boolean isSimpleArray = false, isComplexArray = false; // True only
                                                                    // if
                                                                    // current
                                                                    // parameter
                                                                    // is an
            ...
            if (root_obj.hasNext()) {
                ...
                if (next_elem instanceof org.apache.axis.message.Text) {
                    ...
                    curr_param = new org.apache.axis.message.SOAPBodyElement(new QName(cur_param_qname.getLocalPart())); // can't
                                                                                                                            // touch
                                                                                                                            // this
                    curr_param.addTextNode(inner_txt.getNodeValue());
                }

                // 2nd case) Parameter is represented as a list of attributes
                // (complex type, simple type array, complex type array)
                    ...
                    while (objs_to_add.hasNext()) {
                        ...
                        if (next_to_add instanceof org.apache.axis.message.Text) {
                            ...
                            isSimpleArray = next_to_add_name.contains("response"); // clause
                                                                                    // for
                                                                                    // simple
                                                                                    // types
                            isComplexArray = elem.getNodeName().contains("Response"); // clause
                                                                                        // for
                                                                                        // complex
                                                                                        // types
                            ...
                            if (!isSimpleArray && !isComplexArray) {
                                ...
                                // Then 'elem' belongs to a complex type, and it
                                // is already in a suitable format to add to the
                                // request
                                ...
                                addSimpleArrayToRequest(next_to_add_soap, curr_param, cur_param_qname.getLocalPart(),
                                    operation);
                            ...
                            throw new Exception("Unknown return class '" + next_to_add.getClass().getCanonicalName()
                                + "'");
            ...
            if (isSimpleArray)
                continue paramsLoop; // Array is completely set, proceed to
                                        // next parameter
        ...
        /*
         * System.out.println("Printing SOAP Envelope for
         * "+workflowDescriptor.getOperationQName().toString() +":
        ...
        // "+workflowDescriptor.getOperationQName().toString()); // DEBUG
        javax.xml.rpc.Service service = null;
        if (hasCredential) { // secure invocation when a credential is
                                // provided
     ...
     * 
     * @param complex_array_elems
     *            Array's complex type elements
     * @param curr_param
     *            The parameter associated with the complex array
     */
     ...
     * 
     * @param first_element
     *            Enclosing tag of the array (probably a SOAP Response)
     * @param curr_param
     *            Parameter associated with the array
     * @param curr_param_name
     *            Name of the parameter associated with the array
     * @param operation
     *            Tag in which the array will be put
     */
    private static void addSimpleArrayToRequest(SOAPElement first_element, SOAPBodyElement curr_param,
        final String curr_param_name, SOAPBodyElement operation) {
        ...
        if (first_element.hasChildNodes()) {
            ...
            while (array_elements.hasNext()) {
                ...
                curr_param = new org.apache.axis.message.SOAPBodyElement(new QName(curr_param_name)); // can't
                                                                                                        // touch
                                                                                                        // this
                ...
                if (next_array_elem instanceof org.apache.axis.message.Text) {
                    ...
                    try {
                        curr_param.addTextNode(inner_txt.getNodeValue());
                        operation.addChildElement(curr_param); // Adding array
                                                                // element to
                                                                // the <Request>
                                                                // element
                    } catch (SOAPException e) {
                    ...
                    // DEBUG
                    // System.out.println("Adding complex array element: "+
                    // next_array_elem);
                }
    ...
    public static String applyXPathQuery(String xml_doc, String xpath_query, QName[] namespaces) {
        ...
        // DEBUG
        // System.out.println("=== Applying query "+xpath_query+" on
        // '"+xml_doc+"'\n");
        ...
        try {
            ...
            /*
             * Execute the query and post process the result so we return
            ...
            if (xpath_result.getLength() <= 1) {
                ...
                /*
                 * In case of a simple type return, it will be a structure like
                 * <FooResponse><response>bar</response><FooResponse> but we
                 * must return only the text node, because the entire tree will
                 * be confounded with a complex type's object
                 */
                if (query_result.hasChildNodes()) {
                    ...
                    if (first_child.getNodeName().equals("response")) {
                        ...
                        // If query_result has only one child, it represents a
                        // simple type. If it has more than that, it represents
                        // an array
                ...
                // ");
                // Arrays are supposed to be into an enclosing tag. So, add an
                // artificial enclosing tag to the array
                result = "<FakeResponse>";
    ...
    public static void main(String[] args) {
        ...
        String xml_response3 = "<GetArrayResponse xmlns=\"http://createarrayservice.introduce.cagrid.org/CreateArrayService\">"
        ...
        try {
            // Please! Do not remove the code below, we should leave them here
            // to work as
            // examples. If you want call othe types of services comment these
            // codes
             ...
             * info.setIntroduce_generated(false); ParameterSpec params[] = new
             * ParameterSpec[]{ new ParameterSpec("0", "inputArray", "", "
             * *Valor de a1* "), new ParameterSpec("1", "inputArray", "", "
             * *Valor de a2* ") };
             */
             ...
             * info.setIntroduce_generated(false); ParameterSpec params[] = new
             * ParameterSpec[]{ new ParameterSpec("0", "pinto", "xsd:string",
             * "lau") };
             */
            // 3) Calling a service with two arrays as parameters and a simple
            // int
            /*
             ...
             * ParameterSpec[]{ new ParameterSpec("0", "inputArray", "", "
             * *Valor de a1* "), new ParameterSpec("1", "inputArray", "", "
             * *Valor de a1* "), new ParameterSpec("2", "inputIntValue", "",
             * "5"), new ParameterSpec("3", "inputIntArray", "", "4"), new
             * ParameterSpec("4", "inputIntArray", "", "3") }; InputInfo in_desc =
             * new InputInfo(params); String retval =
             * ServiceInvocationUtil.generateRequest(info, in_desc);
