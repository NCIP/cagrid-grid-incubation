        ...
        <xsl:choose>
            ...
            <xsl:when test="contains(normalize-space(/q:query/q:src/text()), ',')">
                ...
                <xsl:variable name="bioportalURL">
                    <xsl:value-of select="concat(/q:query/q:serviceUrl, '/search/',/q:query/q:term,'?email=rakesh.dhaval@osumc.edu&amp;','pagesize=20&amp;pagenum=0&amp;')" disable-output-escaping="yes"/>
                    <xsl:text>ontologyids=1350</xsl:text>
                </xsl:variable>
