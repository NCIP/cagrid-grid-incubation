<xsl:stylesheet xmlns:xlink="http://www.w3.org/1999/xlink"
	...
	<xsl:template match="searchBean">
		<concept>
			<names>
				<id>
					rest.bioontology.org_BioPortal-SNOMEDCT_
			...
			<definitionURL>
				...
				<xsl:attribute name="xlink:href">
					<xsl:value-of
					select="concat(definitionRef,'/',ontologyId,'/',encode-for-uri(conceptIdShort))" />
                </xsl:attribute>
			</definitionURL>
			<htmlURL>
				<xsl:attribute name="xlink:type">
				...
				<xsl:attribute name="xlink:href">
                    <xsl:value-of
					select="concat(htmlURL,'/',ontologyId,'/','?conceptid=',conceptIdShort)" />
                </xsl:attribute>
			</htmlURL>
			<original_data>
				<ontologyId>
