 ...
 * 
 */
public abstract class TideContextResourceBase extends ReflectionResource implements Resource
                                                  ,PersistenceCallback
                                                  ,TopicListAccessor
                                                  ,SecureResource
                                                  ,RemoveCallback
	...
	static final Log logger = LogFactory.getLog(TideContextResourceBase.class);

	private TideContextResourceConfiguration configuration;
    ...
    public TideContextResourceBase() {
        try {
            resourcePropertyPersistenceHelper = new gov.nih.nci.cagrid.introduce.servicetools.XmlPersistenceHelper(TideContextResourceProperties.class,TideConfiguration.getConfiguration());
            resourcePersistenceHelper = new FilePersistenceHelper(this.getClass(),TideConfiguration.getConfiguration(),".resource");
        } catch (Exception ex) {
	...
	public void setTerminationTime(Calendar time) {	
		Topic terminationTopic = ((Topic)getResourcePropertySet().get(TideContextConstants.TERMINATIONTIME));
	...
	public org.cagrid.tide.descriptor.TideDescriptor getTideDescriptor(){
		return ((TideContextResourceProperties) getResourceBean()).getTideDescriptor();
	}
	
	public void setTideDescriptor(org.cagrid.tide.descriptor.TideDescriptor tideDescriptor ) throws ResourceException {
        ResourceProperty prop = getResourcePropertySet().get(TideContextConstants.TIDEDESCRIPTOR);
		prop.set(0, tideDescriptor);
        //call the first store to persist the resource
        store();
	}
	...
	public TideContextResourceConfiguration getConfiguration() {
		...
		try {
			...
			this.configuration = (TideContextResourceConfiguration) initialContext.lookup(jndiName);
		} catch (Exception e) {
    ...
    public void load(ResourceKey resourceKey) throws ResourceException, NoSuchResourceException, InvalidResourceKeyException {
	  beingLoaded = true;
	   ...
	   TideContextResourceProperties props = (TideContextResourceProperties)resourcePropertyPersistenceHelper.load(TideContextResourceProperties.class, resourceKey.getValue());
       this.initialize(props, TideContextConstants.RESOURCE_PROPERTY_SET, resourceKey.getValue());
