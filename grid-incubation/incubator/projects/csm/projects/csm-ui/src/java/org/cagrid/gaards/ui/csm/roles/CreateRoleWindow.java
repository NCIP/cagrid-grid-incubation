...
public class CreateRoleWindow extends JDialog {
     ...
     */
    public CreateRoleWindow(Application application) {
    ...
    private void createRole() {
        try {
            getProgress().showProgress("Creating role...");
            create.setEnabled(false);
            this.application
                .createRole(Utils.clean(getRoleName().getText()), Utils.clean(getDescription().getText()));
            roleCreated = true;
            getProgress().stopProgress("");
            dispose();
            GridApplication.getContext().showMessage("Successfully created the role.");
        } catch (Exception e) {
    ...
    private JPanel getGroupPanel() {
        if (groupPanel == null) {
            ...
            groupPanel.add(getRoleName(), gridBagConstraints7);
    ...
    private JPanel getTitlePanel() {
        if (titlePanel == null) {
            titlePanel = new TitlePanel("Create Role",
                "Create a role for the application "+application.getName()+".");
        }
        return titlePanel;
    }
