package org.cagrid.sdkquery43.extension.translator;
 ...
 */
public class ExtendedCQL2ToParameterizedHQL {
    public static final String TARGET_ALIAS = "__TargetAlias__";
    
    private static Log LOG = LogFactory.getLog(ExtendedCQL2ToParameterizedHQL.class);
    ...
    public ExtendedCQL2ToParameterizedHQL(Cql2TypesInformationResolver typesInfoResolver, 
	...
	/**
	 * Applies sort and limit query modifier extensions to the HQL query
	 * 
	 * @param mods
	 * 		The modifiers extensions to apply
	 * @throws QueryTranslationException 
	 * @throws QueryTranslationException 
	...
	private StringBuilder handleQueryModifierExtension (CQLQueryModifier mods, StringBuilder hql) throws QueryTranslationException {
		...
		try {
			String qmExtensionAnyNodeString = AnyNodeHelper.convertAnyNodeToString(qmExtensionAnyNode);

	        SAXBuilder builder = new SAXBuilder();
	        InputSource inStream = new InputSource();

	        inStream.setCharacterStream(new java.io.StringReader(qmExtensionAnyNodeString));
	        
	        // Switch to using JDOM to Parse
	        ...
	        // Get Attribute names if they exist
	        Element distinctElement = root.getChild("DistinctAttribute", namespace);
	        List<Element> namedElements = root.getChildren("NamedAttribute", namespace);
	        
	        StringBuilder prepend = new StringBuilder();
	        // if Distint Attributes exist
	        if (distinctElement != null){
	        	String distinctAttribute = distinctElement.getAttributeValue("attributeName");
	        	prepend.append("select distinct ");	        	
	        	prepend.append(distinctAttribute);
	        	prepend.append(' ');
	        	
	        	// Build DistinctAttribute for results processing
	        	Aggregation distinctAggregation = null;
	        	String aggregationValue = distinctElement.getAttributeValue("aggregation").toUpperCase();
	        	if (aggregationValue != null){
	        		distinctAggregation = Aggregation.fromString(aggregationValue);
	        	}
	        	DistinctAttribute da = new DistinctAttribute();
	        	da.setAggregation(distinctAggregation);
	        	da.setAttributeName(distinctAttribute);
	        	mods.setDistinctAttribute(da);
	        	
	        } else if (namedElements != null && namedElements.size() > 0){
	        // if Named Attributes exist
	        	prepend.append("select distinct ");
	        	
	        	//add in extra id to match schema
	        	Element extra = new Element("NamedAttribute", namespace);
	        	extra.setAttribute("attributeName", "id");
	        	namedElements.add(extra);
	        	mods.setNamedAttribute(new NamedAttribute[namedElements.size()]);
	        	
	        	for (int i = 0; i < namedElements.size(); i++) {
	        		String attributeName = namedElements.get(i).getAttributeValue("attributeName");
					prepend.append(attributeName);
					if (i + 1 < namedElements.size()) {
						prepend.append(", ");
					}
					mods.setNamedAttribute(i, new NamedAttribute(attributeName));
				}
	        	prepend.append(' ');	        	
	        } else {
		    // if no attributes are listed
    ...
    private void appendNamedJoins(NamedAssociation na, String parentClassName, 
        ...
        try {
            ...
            throw new QueryTranslationException("Error determining available associations: ", ex);
        }
