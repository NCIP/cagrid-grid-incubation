...
let $target-node-name as xs:string := request:get-parameter("node", "")
...
let $results as element()* :=
   (
      ...
      order by $name
      return <result id="{$id}" target-node-name="{$target-node-name}">{$name}</result>
   )
...
return
   transform:stream-transform(
      ...
      concat("xmldb:exist://" ,lib-util:formsPath(), "/x-forms-popup.xsl"),
      <parameters/>)
