...
import org.cagrid.tide.common.TideConstants;
import org.cagrid.tide.stubs.TideResourceProperties;
 ...
 * 
 */
public class TideResourceHome extends SingletonResourceHomeImpl implements Initializable {

	static final Log logger = LogFactory.getLog(TideResourceHome.class);
    private static final UUIDGen UUIDGEN = UUIDGenFactory.getUUIDGen();

	public Resource createSingleton() {
		logger.info("Creating a single resource.");
		try {
		    TideResourceProperties props = new TideResourceProperties();
			TideResource resource = new TideResource();
			if (resource instanceof PersistenceCallback) {
                  ...
                  try{
                    ((PersistenceCallback) resource).load(null);
			          ...
			          resource.initialize(props, TideConstants.RESOURCE_PROPERTY_SET, UUIDGEN.nextUUID());
			      }
            } else {
                    resource.initialize(props, TideConstants.RESOURCE_PROPERTY_SET, UUIDGEN.nextUUID());
            }
	...
	public Resource find(ResourceKey key) throws ResourceException {
		TideResource resource = (TideResource) super.find(key);
		return resource;
	}
     ...
     */
    public TideResource getAddressedResource() throws Exception {
        TideResource thisResource;
        thisResource = (TideResource) ResourceContext.getResourceContext().getResource();
        return thisResource;
    }
}