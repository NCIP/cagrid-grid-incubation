...
return
   lib-rendering:txfrm-webpage("Supported Object Classes", 
   element object_classes {
      let $regexp := concat("^(", $letter, ").*")
      
      for $item in lib-util:mdrElements('object_class')
         let $id := data(lib-util:mdrElementId($item))
         let $name:= administered-item:preferred-name($item)
         let $anchor := <a xmlns="http://www.w3.org/1999/xhtml" href='../web/object_class.xquery?compound_id={$id}'>{$name}</a>
