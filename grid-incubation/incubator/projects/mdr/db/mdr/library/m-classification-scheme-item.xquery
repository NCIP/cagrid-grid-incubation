module namespace classification-scheme-item="http://www.cagrid.org/xquery/library/classification-scheme-item";
...
declare function classification-scheme-item:resolved-instance($identifier as xs:string) as node()
{
    ...
    let $term_name := $document//openMDR:classification_scheme_item_value
    ...
    <term>
        ...
        <name>{$term_name}</name>
        {
        for $related_term in $document//openMDR:association
        return
        <related_term>
            <identifier>{data($related_term/@associationTarget)}</identifier>
            <relation>{$related_term//openMDR:classification_scheme_item_relationship_type_description}</relation>
        </related_term>
        }
    </term>
};
...
(:as node():)
{
    ...
    let $classification_scheme_identifier := string($triples//rdf:Description[@rdf:about = $scheme]/openMDR:administrationRecord/@rdf:resource)
