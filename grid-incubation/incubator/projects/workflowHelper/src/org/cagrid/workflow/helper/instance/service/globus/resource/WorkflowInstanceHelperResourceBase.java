...
public abstract class WorkflowInstanceHelperResourceBase extends ReflectionResource implements Resource
	...
	public org.cagrid.workflow.helper.descriptor.WorkflowInstanceHelperDescriptor getWorkflowInstanceHelperDescriptor(){
		return ((WorkflowInstanceHelperResourceProperties) getResourceBean()).getWorkflowInstanceHelperDescriptor();
	}
	...
	public WorkflowInstanceHelperResourceConfiguration getConfiguration() {
		...
		servicePath+="/WorkflowInstanceHelper";
		...
		try {
			...
			this.configuration = (WorkflowInstanceHelperResourceConfiguration) initialContext.lookup(jndiName);
		} catch (Exception e) {
    ...
    public void refreshRegistration(boolean forceRefresh) {
        if (getConfiguration().shouldPerformRegistration()) {
            ...
            if (registrationFile.exists() && registrationFile.canRead()) {
                logger.info("Loading registration argumentsrmation from:" + registrationFile);
