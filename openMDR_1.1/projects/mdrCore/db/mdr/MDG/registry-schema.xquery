...
declare function local:local-name($admin-item as element()) as xs:string
{
    ...
    return
        if (empty($admin-item//openMDR:field_name[@preferred='true']/text()))
        then local:normalised-name($name)
        else (xs:string($admin-item//openMDR:field_name[@preferred='true']/text()))
};
...
return
        ...
        <xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema" 
            ...
            {
            for $value-domain in lib-util:mdrElements("value_domain")[.//openMDR:registration_status/text() ne 'Superseded']
            ...
            where exists(//@permissible_value_identifier) or $value-domain//openMDR:value_domain_datatype/text() = xs:string($datatype/@datatype_identifier)
            return 
                <xs:simpleType sawsdl:modelReference="{$sawsdl}" name="{$local-name}">
                    <xs:restriction base="{$datatype/openMDR:datatype_name}">
                        {if ($value-domain//openMDR:value_domain_format/text() > "") 
                        then (<xs:pattern value="{$value-domain//openMDR:value_domain_format}"/>)
                        else ()}
                        {if ($value-domain//openMDR:value_domain_maximum_character_quantity/text() > "")
                        then (<xs:maxLength value="{$value-domain//openMDR:value_domain_maximum_character_quantity}"/>)
                        else ()}
                        {
                        for $containing in $value-domain/openMDR:containing
                        return
                            <xs:enumeration value="{$containing/openMDR:value_item}"/>
                        }
                     </xs:restriction>
                </xs:simpleType>
                ,
            ...
            for $data-element in lib-util:mdrElements('data_element')[.//openMDR:registration_status/text() ne 'Superseded']
            for $value-domain in lib-util:mdrElements('value_domain')[.//openMDR:registration_status/text() ne 'Superseded']
            ...
            where $data-element//openMDR:representing/text() = lib-util:mdrElementId($value-domain)
